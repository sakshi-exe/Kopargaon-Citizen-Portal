...
export function VideoEvidencePlayer({
  ...
  return (
    <div className={`rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-md overflow-hidden ${className}`}>
      
      {/* Header Bar */}
      <div className="px-5 py-3.5 border-b border-slate-100 dark:border-slate-700 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700">
        ...
        {isPlaying ? (
          <div className="w-full h-full relative">
            ...
            <button
              ...
              className="absolute top-3 right-3 px-2.5 py-1 rounded-lg bg-black/70 hover:bg-black/90 text-white text-xs font-semibold flex items-center gap-1 z-10 transition-colors backdrop-blur-sm"
              title="Close Player"
            >
              <X size={13} /> Close
            </button>
          ...
          <div className="relative w-full h-full cursor-pointer" onClick={() => setIsPlaying(true)}>
            ...
            <img
              ...
              onError={(e) => {
                // Fallback to high quality infrastructure photo if YouTube thumbnail fails to load
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="relative flex items-center justify-center">
                <div className="absolute w-20 h-20 rounded-full bg-teal-500/30 animate-ping pointer-events-none" />
                <button
                  type="button"
                  className="w-16 h-16 rounded-full bg-teal-500 hover:bg-teal-400 text-slate-950 flex items-center justify-center shadow-2xl shadow-teal-500/50 group-hover:scale-110 transition-all z-10 pl-1"
                >
                  <Play size={28} className="fill-current text-slate-950" />
                </button>
              </div>
              <span className="mt-3 text-xs font-extrabold uppercase tracking-widest text-white drop-shadow-md bg-slate-900/80 px-3 py-1 rounded-full border border-white/20">
            ...
            <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between text-white pointer-events-none">
              <div>
                <div className="text-[11px] font-bold text-teal-400 flex items-center gap-1 mb-0.5">
      ...
      {/* Footer Info Cards */}
      <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700/60 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        <div>
          <span className="text-slate-500 dark:text-slate-400 text-[10px] font-medium block">Project Linked</span>
          <span className="font-bold text-slate-800 dark:text-slate-200 truncate block mt-0.5">{projectName}</span>
        </div>
        <div>
          <span className="text-slate-500 dark:text-slate-400 text-[10px] font-medium block">Audit Location</span>
          <span className="font-bold text-slate-800 dark:text-slate-200 block mt-0.5">{location}</span>
        </div>
        <div>
          <span className="text-slate-500 dark:text-slate-400 text-[10px] font-medium block">Verified Authority</span>
...
export function CompactVideoEvidenceCard({
  ...
  return (
    <>
      <div
        onClick={handlePlay}
        className="group relative rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 hover:border-teal-500 dark:hover:border-teal-400 hover:shadow-md transition-all cursor-pointer overflow-hidden"
      >
        <div className="flex items-center gap-3">
          {/* Mini Thumbnail */}
          <div className="relative w-24 h-16 rounded-lg overflow-hidden flex-shrink-0 bg-slate-900">
            <img
              src={THUMBNAIL_URL}
            ...
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <div className="w-7 h-7 rounded-full bg-teal-500 text-slate-950 flex items-center justify-center group-hover:scale-110 transition-transform pl-0.5 shadow">
        ...
        >
          ...
          >
            ...
            <div className="p-4 text-xs text-slate-300 flex items-center justify-between">
              ...
              <button
                onClick={() => setModalOpen(false)}
                className="px-4 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs"
              >
                Done
              </button>
            </div>
