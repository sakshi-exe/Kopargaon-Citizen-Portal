...
import { Search, MapPin, QrCode, ShieldCheck, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Header
        ...
        subtitle="Geospatial municipal infrastructure, active development projects, and public tickets"
      />

      {/* Map Search & Filter Bar */}
      <div className="px-4 sm:px-6 py-2.5 bg-white/95 dark:bg-[#0B132B]/95 backdrop-blur-md border-b border-slate-200/90 dark:border-slate-800/90 flex flex-wrap items-center justify-between gap-3 flex-shrink-0 z-20">
        <div className="relative flex-1 min-w-[260px] max-w-md">
          ...
          <input
            ...
            placeholder="Search road, ward, infrastructure asset ID..."
          ...
          {filteredAssets.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1.5 bg-white dark:bg-[#0F172A] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl z-50 max-h-60 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800">
              {filteredAssets.map(asset => (
                ...
                >
                  ...
                  <span className="text-[10px] text-slate-400 font-semibold px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 flex-shrink-0">{asset.type}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex items-center gap-2 sm:gap-3 text-xs text-slate-600 dark:text-slate-400 overflow-x-auto">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-300 text-[11px] font-semibold whitespace-nowrap">
            ...
            <span>Tickets (<strong>{analytics.issues.total}</strong>)</span>
          </div>
        </div>
      </div>
