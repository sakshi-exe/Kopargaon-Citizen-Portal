...
function LocationPicker({ onPick, picked }) {
  useMapEvents({
  ...
  const icon = L.divIcon({
    html: `<div style="width:20px;height:20px;border-radius:50%;background:#ef4444;border:3px solid white;box-shadow:0 0 0 3px rgba(239,68,68,0.3)"></div>`,
    className: '',
    iconSize: [20, 20],
    iconAnchor: [10, 10],
  });

  return null; // marker added via effect inside map
}
...
export default function ReportIssue() {
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Issue Reported Successfully</h2>
          <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-6">
            Your report has been submitted and is now visible on the city map. You can track its status under <strong>My Reports</strong>.
          </p>
          <div className="flex gap-3">
            <button onClick={() => setSubmitted(false)} className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700">
              Report Another Issue
            </button>
            <a href="/citizen/my-reports" className="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800">
              View My Reports
            </a>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Header title="Report a Civic Issue" subtitle="Help us improve the city by reporting problems in your area" />
        ...
        <div className="max-w-4xl mx-auto p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                {pickedLoc ? (
                  <div className="mt-2 flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400">
                    <MapPin size={12} />
                    Location selected: {pickedLoc[0].toFixed(5)}, {pickedLoc[1].toFixed(5)}
                  </div>
                ) : (
                  <p className="mt-2 text-xs text-slate-500">Click on the map to mark the exact issue location</p>
                )}
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => { setForm({ category:'',description:'',wardId:'',citizenName:'',isAnonymous:false,photo:null }); setPickedLoc(null); setErrors({}); }}
