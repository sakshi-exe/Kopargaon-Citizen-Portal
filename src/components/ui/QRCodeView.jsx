...
export function AssetQRBadge({ assetId, assetName, size = 'md', onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex flex-col items-center justify-center p-2.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-xs ${
        onClick ? 'cursor-pointer hover:border-navy-400 dark:hover:border-navy-500 hover:shadow-md transition-all' : ''
      }`}
    >
      ...
      <div className="mt-1.5 text-center font-mono font-bold text-slate-900 dark:text-white tracking-wider">
        <span className={config.text}>{assetId}</span>
      </div>
      <div className="text-[9px] text-emerald-600 dark:text-emerald-400 font-semibold tracking-tight uppercase">
        ✓ CIVICFIX VERIFIED
      </div>
    </div>
  );
