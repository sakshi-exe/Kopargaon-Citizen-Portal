  ...
  return (
    <aside className={`flex h-full flex-col border-r border-[#F0DCC0] bg-[#FFFDF8] transition-all duration-200 dark:border-slate-800 dark:bg-[#091827] ${collapsed ? 'w-20' : 'w-72'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              ...
              <div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#52627A] dark:text-[#A9C2F7]">Admin command</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="px-4 pb-2 pt-5">
        {!collapsed && (
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#52627A] dark:text-[#A7B9D9]">Municipal console</div>
        )}
      </div>
      ...
      <div className="border-t border-[#F3E4D2] px-3 py-4 dark:border-slate-800">
        ...
        >
          <span className="flex items-center gap-2">
            <ChevronLeft size={15} className="text-[#138808]" />
            {!collapsed && 'Switch to citizen'}
          </span>
        </button>
      </div>
    </aside>
